    ...
    match task {
        ...
        Some("dist") => {
            ...
            command(
                root,
                "cargo",
                &["build", "--release", "--locked", "-p", "mammoth-cli", "-p", "mammoth-mcp"],
            )?;
            package(root, None)?;
        }
...
fn package(root: &Path, target: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mcp_binary = if target.contains("windows") { "mammoth-mcp.exe" } else { "mammoth-mcp" };
    let mcp_source = build.join("release").join(mcp_binary);
    let mcp_hash = sha256(&mcp_source)?;
    ...
    std::fs::copy(mcp_source, package.join(mcp_binary))?;
    ...
    let info = serde_json::json!({
        ...
        "mcp_binary_sha256": mcp_hash,
        "scope": "durable coding-agent memory, local preview; see AGENT-MEMORY.md"
    });
    std::fs::write(
